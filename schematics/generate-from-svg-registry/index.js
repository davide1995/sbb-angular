    ...
    _generateIcons(icons, targetDir) {
        return schematics.chain(icons.map(i => schematics.mergeWith(schematics.apply(schematics.url('./files/icon'), [
            schematics.template({
                ...
                ...i,
                prefix: this._prefix,
                cssClasses: i.collections.map(c => `${this._prefix}-${c}`).join(' '),
...
function generateFromSvgRegistry(options) {
    return async (tree, context) => {
        ...
        if (project.projectType === 'library') {
            ...
            for (const rule of rules) {
                await schematics.callRule(rule, tree, context).toPromise();
            }
        }
    };
}

exports.generateFromSvgRegistry = generateFromSvgRegistry;
